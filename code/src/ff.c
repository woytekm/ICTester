...
FRESULT dir_next (	/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:EOT and could not streach */
	DIR *dj,		/* Pointer to directory object */
	BOOL streach	/* FF_FALSE: Do not streach table, FF_TRUE: Streach table if needed */
)
{
	DWORD clst;
	WORD i;
...
static
BOOL cmp_lfn (			/* FF_TRUE:Matched, FF_FALSE:Not matched */
...
)
{
	...
	do {
		...
		if (wc) {	/* Last char has not been processed */
			...
			if (i >= _MAX_LFN || wc != ff_wtoupper(lfnbuf[i++]))	/* Compare it */
				return FF_FALSE;			/* Not matched */
		} else {
			if (uc != 0xFFFF) return FF_FALSE;	/* Check filler */
		}
	...
	return FF_TRUE;					/* The part of LFN matched */
}



static
BOOL pick_lfn (			/* FF_TRUE:Succeeded, FF_FALSE:Buffer overflow */
...
)
{
	...
	do {
		uc = LD_WORD(dir+LfnOfs[s]);			/* Pick an LFN character from the entry */
		if (wc) {	/* Last char has not been processed */
			if (i >= _MAX_LFN) return FF_FALSE;	/* Buffer overflow? */
...
)
{
	...
	do {
		...
		if (!(dir[DIR_Attr] & AM_VOL) && !mem_cmp(dir, dj->fn, 11)) /* Is it a valid entry? */
			break;
#endif
		res = dir_next(dj, FF_FALSE);		/* Next entry */
	} while (res == FR_OK);
...
)
{
	...
	do {
		...
		res = dir_next(dj, FF_TRUE);	/* Next entry with table streach */
	} while (res == FR_OK);
	...
	if (res == FR_OK) {
		...
		if (!fno) {
			...
			if (res == FR_OK) {				/* A valid entry is found */
				...
				res = dir_next(dj, FF_FALSE);	/* Increment index for next */
				if (res == FR_NO_FILE) {
					dj->sect = 0;
					res = FR_OK;
				}
