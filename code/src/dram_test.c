...
void clear_aliases(dram_test_settings_t *dram_test_settings)
 {
  uint8_t i;
...
void set_aliases_dual_port(dram_test_settings_t *dram_test_settings)
 {
  ...
  for(i = 0; i < dram_test_settings->data_width; i++)
   {
    sprintf(alias,"D(I)%d",i);
    strncpy(dram_test_settings->pin_aliases[i+20], alias, 4);
   }

  for(i = 0; i < dram_test_settings->data_width; i++)
   {
    sprintf(alias,"O(Q)%d",i);
    strncpy(dram_test_settings->pin_aliases[i+30], alias, 4);
   }
...
void init_dram_test(uint8_t addr_width, uint8_t data_width, bool ce, bool we, bool oe, bool ras, bool cas, uint16_t loops, dram_test_settings_t *dram_test_settings)
{
  ...
  dram_test_settings->ce_active = ce;
  dram_test_settings->we_active = we;
  dram_test_settings->oe_active = oe;
  dram_test_settings->ras_active = ras;
  dram_test_settings->cas_active = cas;
  dram_test_settings->loops = loops;
  dram_test_settings->dram_type = SINGLE_PORT;

  for(i = 0; i < PIN_COUNT; i++)
   dram_test_settings->pin_aliases[i][0]=0x0;

  set_aliases_single_port(dram_test_settings); 
...
void test_dram_early_write(uint8_t dram_type, uint16_t addr_max, uint8_t pattern, bool alternate_pattern)
 {

  uint16_t caddr,raddr;
  uint8_t data_pattern = pattern,caddr_low,caddr_high,raddr_low,raddr_high;
  uint8_t o_bank = DR_DATA_BANK;  // in dual data port configuration: bank 2: I, bank 3: O

  if(dram_type == DUAL_PORT)
   o_bank = DR_ADDR_HIGH_BANK;

  set_direction_bank(o_bank, PIN_OUTPUT);

  if(dram_type == SINGLE_PORT)
      ...
      for(caddr = 0; caddr < addr_max; caddr++)
       {
         ...
         set_level_bank(o_bank,data_pattern);    // enter data on data lines
...
void test_dram_read_mod_write(uint8_t dram_type, uint16_t addr_max)
 {
      ...
      for(caddr = 0; caddr < addr_max; caddr++)
       {
         ...
         set_level_bank(DR_CTRL_BANK,0b11100110);   // CE=L, WE=H, OE=L, CAS=L, RAS=L  - OE goes H again
         input = read_level_bank(DR_DATA_BANK);     // read in the data - it should be latched on data bus
         set_direction_bank(o_bank, PIN_OUTPUT); // change data bus direction
         set_level_bank(o_bank,(~input));       // set inverted byte back on the data bus
         set_level_bank(DR_CTRL_BANK,0b11100100);   // activate WE (active low) - latch modified data into DRAM
         set_level_bank(DR_CTRL_BANK,0b11100110);   // WE goes H again
         set_level_bank(DR_CTRL_BANK,0b11110110);   // CAS goes H
         set_level_bank(DR_CTRL_BANK,0b11111110);   // RAS goes H - end of R-M-W cycle
...
uint32_t test_dram_page_read(uint16_t addr_max, uint8_t data_bitmask, uint8_t pattern, bool alternate_pattern)
 {
...
void test_dram_page_write(uint8_t dram_type, uint16_t addr_max, uint8_t pattern, bool alternate_pattern)
 {
...
uint32_t test_dram_read(uint8_t dram_type, uint16_t addr_max, uint8_t data_bitmask, uint8_t pattern, bool alternate_pattern)
 {
